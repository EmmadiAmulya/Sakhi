...
/**
 * Client-side read cache (Zustand + persist).
 *
 * IMPORTANT: this store no longer writes to Supabase. All persistence flows
 * through the TanStack Query hooks in `src/lib/data/*`, which do optimistic
 * updates against this store, roll back on error, and surface a toast. The store
 * actions here are pure local-state mutations used by those hooks and for
 * hydration; the views call the hooks, never these actions, for writes.
 */
  ...
  persist(
    (set) => ({
      ...
      setCycleLog: (date, log) =>
        set((state) => {
          const existing = state.cycleLogs[date] || { date, isPeriod: false, symptoms: [] };
          return {
            cycleLogs: { ...state.cycleLogs, [date]: { ...existing, ...log } },
          };
        }),
      ...
      addJournalEntry: (entry) => {
        ...
        const newEntry: JournalEntry = { id, createdAt, ...entry };
      ...
      updateReminders: (prefs) =>
        set((state) => ({ reminders: { ...state.reminders, ...prefs } })),
    }),
    {
